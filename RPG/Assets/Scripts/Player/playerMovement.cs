...
public class playerMovement : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetKeyDown ("c") && hudScript.spell == false) {
			if (!crouch) {
				col.height = scale;
				crouch = true;
			} else {
	...
	void FixedUpdate()
	{
		...
		if (Physics.BoxCast (transform.position, new Vector3 (0.74f, 0.9f, 0.74f), Vector3.down, out hit, transform.rotation, jumpray)  && hudScript.spell == false) {
